...
const BASE_URL = 'http://159.89.168.252/api/products/products/';
...
export async function getCategories() {
  const token = localStorage.getItem('token');
  const response = await axios.get('http://159.89.168.252/api/products/categories/', {
    headers: {
      Authorization: `Bearer ${token}`,
    },
  });
...
export async function getLocations() {
  const token = localStorage.getItem('token');
  const response = await axios.get('http://159.89.168.252/api/products/locations/', {
    headers: {
      Authorization: `Bearer ${token}`,
    },
  });
